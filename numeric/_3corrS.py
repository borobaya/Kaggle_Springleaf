...
def do(a, b, aTarget):
    ...
    if False:
        ...
        colCorrs = helper.corrMat(a, use_spearman=True)
        print("Saving...")
        with open("../stats/colCorrsS.pkl", "w") as f:
            pk.dump(colCorrs, f, protocol=2)
    else:
        with open("../stats/colCorrsS.pkl", "r") as f:
            colCorrs = pk.load(f)

    print("Column count: %d" % (a.columns.size))

    for thres in np.arange(1, 0.99, -0.01):
        print("Columns correlated >=%f..." % (thres))
        groups = helper.getGroups( (colCorrs>=thres) , a, keys)
        a, b = helper.regressWithinGroups(groups, thres, a, b, use_ranks=True)

    return a, b


def run(loadname="2corrP", savename="3corrS"):
    # Load
